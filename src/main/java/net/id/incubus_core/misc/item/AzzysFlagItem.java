...
public class AzzysFlagItem extends HoeItem {
	...
	private void handleGifting(PlayerEntity target, Random random, PlayerEntity user, boolean sneaking) {
		...
		if (sneaking) {
			...
			target.sendMessage(new TranslatableText("You have been graced with gifts!").styled(style -> style.withColor(0xffb0b3)), true);
			for (int i = 0; i < cap; i++) {
		...
		if (target instanceof PlayerEntity player) {
			switch (mode) {
				...
				case WORTHY: {
					...
					if (!worthy) {
						player.sendMessage(new TranslatableText("You have been deemed worthy"), true);
						world.playSoundFromEntity(null, target, IncubusSounds.DRIP_LONG, SoundCategory.PLAYERS, 0.5F, 1F);
					} else {
						player.sendMessage(new TranslatableText("You have no right!"), true);
						world.playSoundFromEntity(null, target, SoundEvents.UI_BUTTON_CLICK, SoundCategory.PLAYERS, 0.5F, 1F);
					}
	...
	public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
		tooltip.add(new TranslatableText("§f§oFor The Worthy"));
		tooltip.add(new TranslatableText("§olady azzy's blushing staff").styled(style -> style.withColor(0xffb0b3)));
