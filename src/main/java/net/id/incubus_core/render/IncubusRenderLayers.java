...
public class IncubusRenderLayers extends RenderLayer {
    ...
    public static final RenderLayer SIMPLE_BLOOM = RenderLayerConstructor.buildMultiPhase(IncubusCore.MODID + ":basic_bloom", VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.QUADS, 256, false, false, MultiPhaseParameters.builder().texture(new Texture(locate("textures/special/blank.png"), false, false)).shader(RenderPhase.SOLID_SHADER).transparency(RenderPhase.TRANSLUCENT_TRANSPARENCY).target(BLOOM_TARGET).build(true));
